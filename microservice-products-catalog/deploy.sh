...
if [ ! -d ".elasticbeanstalk" ]; then
	...
	eb init -p python-3.12 microservice-products-catalog --region ${AWS_REGION} || {
...
if ! eb status microservice-products-catalog &>/dev/null; then
	echo "📦 Creating Elastic Beanstalk Environment ..."
	eb create microservice-products-catalog || {
